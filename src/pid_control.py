...
Important:
  ...
  geometry_msgs.msg ---- be sure it is used on the other nodes --- Teleop and the other controllers
  kingfisher_msgs.msg --- modify or create a new message to contain Drive and Course
  
  yaw_type it is very important because: 
  it switches between 2 types of control each with different subscriber messages
  --------------important to now before configuring the IMU node ------------
  if yaw_rate then rospy.Subscriber("cmd_vel",Twist,node.twist_callback)
  depend on twist message
  if yaw then rospy.Subscriber("cmd_course",Course,node.course_callback)
  depend on quaternions
'''
# Python
import sys
from math import pi
